...
# Seperating for Switzerland Dataset
df = df.iloc[599:720,:]


# SHOWS THE ANALYSIS REPORT
report = sweetviz.analyze([df,'df'],target_feat='target')
report.show_html('switzerland_report.html')
...
# Applying it to Scatter PCA function
mat_reduced,feat = Scatter_PCA(scal_features ,10)
mat_reduced.shape
# np.save("swiz_reduced.npy",mat_reduced)
# np.save("swiz_y.npy",Y)

# Saving the Reduced Features
x_value = np.load("swiz_reduced.npy")
y_value = np.load("swiz_y.npy")
...
y_preds_grid = clf.predict(x_value)
