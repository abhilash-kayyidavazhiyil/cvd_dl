...
#------------------------------------------------------------------------------------------------------------------------
"GENETIC SINE ALGORITHM FOR FEATURE SELECTION"

import numpy as np
from numpy.random import rand
from Algorithm.functionHO import Fun
...
def jfs(xtrain, ytrain, opts):
    ...
    if np.size(lb) == 1:
        ...
        if np.size(lb) < 0.5:
            ...
            ub  = dim[CR,1] + lb * np.sin(ub) * abs(1 , MR[0,dim] - dim[CR,ub]) 
        else:
            lb = dim[CR,1] + lb * np.cos(ub) * abs(1 , MR[0,dim] - dim[CR,lb])
